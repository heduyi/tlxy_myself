...
# 9. 类的内置属性
 ...
 - 属性操作相关
    ...
    - `__setattr__`: 对成员属性进行设置的时候会触发（参考举例）
        - 参数：
            - 1.self用来获取当前对象
            - 2.被设置的属性名称，以字符串形式出现
            - 3.需要对属性名称设置的值
...
- 运算分类相关魔术方法
    - `__gt__`：进行大于判断的时候触发的函数，对2个实例进行大小比较时会触发该魔术方法
        - 参数：
            - 1. self
            - 2. 第二个参数是进行比较的第二个参数
            - 3. 返回值是任意的值，推荐返回布尔值
...
- 抽象类的使用
    - 抽象类可以包含抽象方法，也可以包含具体的传统方法
    - 抽象类中可以有方法也可以有属性
    - 抽象类不允许直接实例化
    - 抽象类必须通过继承才可以使用，即继承的子类必须实现所有继承来的抽象方法
    - 如果抽象类的子类没有实现继承来的所有的抽象方法，则子类也不能实例化
